...
def sdg_from_parts(name_space:NameFinder, state_graphs:list[StateGraph]=None, unbreakable_states:list[State]=None, breakable_states:list[State]=None):
    if state_graphs       is None:
        state_graphs       = tuple()
    if unbreakable_states is None:
        unbreakable_states = tuple()
    if breakable_states   is None:
        breakable_states   = tuple()
    ...
    for state in unbreakable_states:
        ...
        state_graphs.append(StateGraph(name=state_group.get_name(), default_state=state_group))
    for state in breakable_states:
        state_group = StateGroup(name=state.get_name(), states=[state])
        state_graphs.append(StateGraph(name=f"{state.get_name()} Breakable", default_state=state_group, target_graph={state_group: {break_action: broken_group}}))
    return StateDisconnectedGraph(
        name="sdg",
        state_graphs=tuple(state_graphs)
    )
...
def get_wearing(character_name: str, item_limit: ItemLimit) -> LocationDetail:
    return LocationDetail(
        name="wearing",
        description_context=ContentsContext("You are wearing:", "You are wearing nothing of note."),
        description_strategy=ContentsDescription(),
        name_id=f"{character_name} wearing",
        aliases=[f"{character_name} wearing"],
        item_limit=item_limit,
        size=0,
    )
